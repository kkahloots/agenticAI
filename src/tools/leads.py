...
def _balance_factor(balance: float) -> float:
    if balance >= 50_000: return 1.0
    if balance >= 20_000: return 0.7
    if balance >= 5_000:  return 0.4
    return 0.1
...
def _offer_rationale(c: dict, offer_code: str) -> str:
    ...
    return "; ".join(parts) if parts else f"eligible for {_OFFER_LABELS.get(offer_code, offer_code)}"
...
def upsell_recommend(customer_id: str) -> dict:
    ...
    ltv = c.get("lifetime_value", 0)
    eng = c.get("engagement_score", 0)
    upsell_score = round(eng * 0.5 + _balance_factor(ltv) * 0.3 + (1 - c.get("fraud_score", 0.5)) * 0.2, 4)
...
def bulk_recommend(
    ...
    for p in prospects:
        ...
        if flags.get("email", False):
            ...
            blocked_count += 1
            plan.append({**p, "action": "blocked", "reason": "no_channel_consent", "channel": None})
            continue

        if c.get("fraud_score", 0) > cfg.risk_score_threshold:
            approval_needed += 1
            plan.append({**p, "action": "approval_required", "channel": channel,
    ...
    needs_bulk_approval = (sent_count + approval_needed) > cfg.bulk_notification_threshold

...
def run_sql_query(sql: str, params: Optional[dict] = None, max_rows: int = 10_000) -> dict:
    if not _is_safe(sql):
    ...
    try:
        ...
        return {"rows": rows[:max_rows], "row_count": len(rows[:max_rows]), "truncated": truncated}
...
def generate_segment(
    ...
    from src.tools.customer import _load_customers
    ...
    return [
        {
            ...
            "customer_ids": ids,
            "size": len(ids),
            "avg_risk_score": _avg([c["risk_score"] for c in customers if c["customer_id"] in ids and c.get("risk_score") is not None]),
            "avg_engagement_score": _avg([c.get("engagement_score", 0) for c in customers if c["customer_id"] in ids]),
        }
...
def _kmeans_segment(customers: list[dict], n_clusters: int) -> dict:
    ...
    features = [
        [c.get("risk_score") or 0.5, c.get("engagement_score") or 0.5]
        for c in customers
    ]
    X = np.array(features)
    km = KMeans(n_clusters=min(n_clusters, len(customers)), random_state=42, n_init="auto")

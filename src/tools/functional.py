...
def recommend_offer(customer_id: str, context: str = "") -> dict:
    ...
    if not eligible:
        return {"offer_code": None, "offer_name": None, "rationale": "No eligible promotions", "confidence": 0.0}
    ...
    categories = c.get("purchase_categories", [])
    rationale_parts = [f"segment '{c.get('segment')}'",
...
def create_case(
    ...
    for case_id, case in _CASES.items():
        if case["customer_id"] == customer_id and case["case_type"] == case_type and case["status"] == "open":
            return {"case_id": case_id, "status": "open", "assigned_to": case["assigned_to"], "duplicate": True}

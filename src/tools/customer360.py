...
def get_customer_360(customer_id: str) -> dict:
    ...
    """
    from src.tools.customer import search_customer_profile
    ...
    total_sales = sum(s.get("total_amount", 0) for s in sales if s.get("status") == "completed")
    ...
    open_tickets = len([t for t in support if t.get("status") in ["open", "in_progress"]])
    ...
    return {
        ...
        "sales": {
            ...
            "channels": list(set(s.get("channel", "unknown") for s in sales)) if sales else []
        },
        "social": {
            "posts": social,
            "total_count": len(social),
            "sentiment_breakdown": sentiment_counts,
            "platforms": list(set(s.get("platform", "unknown") for s in social)) if social else []
        },
        "support": {
            "tickets": support,
            "total_count": len(support),
            "open_tickets": open_tickets,
            "ticket_types": list(set(t.get("type", "unknown") for t in support)) if support else []
        },
        "summary": {
...
def get_sales_analytics(filters: Optional[dict] = None) -> dict:
    ...
    # Calculate metrics
    total_revenue = sum(s.get("total_amount", 0) for s in sales if s.get("status") == "completed")
    ...
    return {
        ...
        "top_product": max(by_product.items(), key=lambda x: x[1])[0] if by_product else None
    }
...
def get_sentiment_analytics(filters: Optional[dict] = None) -> dict:
    ...
    total = len(social)
    sentiment_percentages = {k: round(v / total * 100, 1) for k, v in sentiment_counts.items()} if total > 0 else {}
...
def get_support_analytics(filters: Optional[dict] = None) -> dict:
    ...
    if filters:
        if "customer_id" in filters:
            tickets = [t for t in tickets if t.get("customer_id") == filters["customer_id"]]
        if "type" in filters:
    ...
    resolved_times = [t.get("resolution_time_hours", 0) for t in tickets if t.get("resolution_time_hours")]

...
- **Intent Detection**: Parse the user's request using an LLM (via the universal LLM factory in `nonagentic/core/llm.py`) to determine whether it is informational, analytical, action‑oriented or strategic.
...
The orchestrator (and all agents) obtain their LLM through a single universal factory at `nonagentic/core/llm.py`. The active provider is controlled by the `LLM_PROVIDER` environment variable — no code changes are needed to switch providers.

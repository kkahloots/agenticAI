...
- All tools are in `src/tools/functional.py` and `src/tools/leads.py`.
...
All approval thresholds are read from environment variables via `src/config.py` — never hardcoded:

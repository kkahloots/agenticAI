# Level 2 Analytics - Documentation

## Overview

Level 2 Analytics is your data science co-pilot. It transforms natural language questions into actionable insights using SQL analytics, machine learning, and NLP.
...
Creates:
- `data/sales_transactions.json` (898 transactions)
- `data/social_media.json` (~79 posts)
- `data/support_tickets.json` (~200 tickets)
...
**What it does**: Automatically discovers customer groups based on risk and engagement scores. Falls back to rule-based segmentation when data is insufficient or sklearn is unavailable.
...
**Safety Features**:
- SQL injection guard rejects INSERT/UPDATE/DELETE/DROP before execution
- Read-only queries enforced
- Row limit controlled by `SQL_MAX_ROWS` env var (default 10,000)
- Schema binding via `create_sql_query_chain`; falls back to raw LLM prompt if DB unavailable
...
```
Level 2 Analytics (src/agents/level2.py)
│
├── SQL Analytics
│   ├── Text-to-SQL (create_sql_query_chain)
│   ├── Query execution (SQLAlchemy, read-only)
│   └── Safety guard (_is_safe rejects mutating statements)
│
├── Machine Learning
...
│   ├── Sentiment analysis (RoBERTa)
│   ├── Text summarization (BART)
│   └── Batch sentiment (analyze_batch_sentiment)
│
├── Customer 360
│   ├── CRM integration
│   ├── Sales data
...
│
├── Visualization
│   └── Chart generation (matplotlib, saved to data/charts/)
│
└── Guardrails
    ├── PII redaction (guardrail_check)
    └── Audit logging
```
...
Run all cells in `notebooks/nonagent/02_level2_analytics.ipynb`
...
- `notebooks/nonagent/02_level2_analytics.ipynb` - Interactive demo
- `specs/nonagent/02-level2-analytics/` - Detailed specifications
- `scripts/generate_customer360_data.py` - Generate sales, social, support data
- `scripts/cust_dataset_generator.py` - Generate customer profiles (208 customers)

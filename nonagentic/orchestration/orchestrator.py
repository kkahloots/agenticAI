...
from nonagentic.orchestration.registry import REGISTRY, describe_all
...
- informational: factual lookup, identity verification status, policy retrieval, customer profile, policy questions about fraud thresholds
- analytical: segmentation, SQL queries, KPIs, churn analysis, fraud risk scoring
...
IMPORTANT: Questions about POLICY (thresholds, rules, eligibility, criteria) are INFORMATIONAL, not analytical.
Questions about FRAUD SCORE THRESHOLDS are POLICY questions and should be classified as INFORMATIONAL.
...
def _rule_classify(request: str) -> tuple[str, float, str | None]:
    ...
    # Informational must run first — document/policy/note queries contain strategic words
    informational_words = [
        "identity",
        "verification",
        "policy",
    ...
    if any(w in r for w in informational_words):
    ...
    if any(
        w in r for w in ["segment", "sql", "query", "kpi", "churn", "analys", "cluster"]
    ):

...
def _llm_fallback_sql(request: str) -> str | None:
    ...
    try:
        ...
        llm = get_llm(temperature=0.0)
        response = llm.invoke([
            SystemMessage(content=f"You are a SQL expert. Generate a safe, read-only SELECT query. {schema}. Return ONLY the SQL, no explanation."),
...
def level2_node(state: AgentState) -> dict:
    ...
    if any(w in request for w in ["segment", "cluster", "group"]):
        ...
        log_audit_event("level2_analytics", "generate_segment", {"filters": filters, "algorithm": algo}, {},
                        user_id=state["user_id"], request_id=state["request_id"])

        from nonagentic.tools.visualisation import visualise
        chart_path = visualise(result.get("segments", []), chart_type="bar",
                               title="Customer Segments", request_id=state["request_id"])
        if chart_path:
        ...
        if sql:
            ...
            log_audit_event("level2_analytics", "run_sql_query", {"sql": sql}, {},
                            user_id=state["user_id"], request_id=state["request_id"])
        else:
            result = {"error": "Could not generate SQL — please rephrase your question."}
    ...
    if not gr.passed:
        log_audit_event("level2_analytics", "guardrail_violation",
    ...
    return {
        ...
        "tool_calls": [{"tool": "level2", "inputs": {"request": state["original_request"]}, "outputs": result}],
        "audit_trail": [{"node": "level2_analytics", "action": "completed",
...
def _format(result: dict) -> str:
    ...
    if "segments" in result:
        ...
        lines = [f"- {s['label']}: {s['size']} customers (avg fraud {s['avg_risk_score']}, avg engagement {s['avg_engagement_score']})" for s in segs]
        warn = f"\n⚠️ {result['warning']}" if "warning" in result else ""
        chart = f"\n📊 Chart saved: {result['chart_path']}" if result.get("chart_path") else ""
        return "Segmentation results:\n" + "\n".join(lines) + warn + chart
    if "rows" in result:
        rows = result["rows"]
        trunc = " (truncated)" if result.get("truncated") else ""
        return f"Query returned {result['row_count']} rows{trunc}:\n" + str(rows[:5]) + ("..." if len(rows) > 5 else "")
    return str(result)

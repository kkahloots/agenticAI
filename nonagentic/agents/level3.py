...
def level3_node(state: AgentState) -> dict:
    ...
    if any(w in request for w in ["lead", "prospect", "score lead", "identify"]):
        ...
        log_audit_event("level3_functional", "score_leads",
                        {"offer_code": offer_code, "segment": segment}, leads,
    ...
    if any(w in request for w in ["bulk", "campaign", "all customers", "segment target"]):
        offer_code = _extract_offer(request)
        segment = _extract_segment(request)
        plan = bulk_recommend(offer_code, segment=segment)
        log_audit_event("level3_functional", "bulk_recommend",
                        {"offer_code": offer_code, "segment": segment}, plan,
                        user_id=state["user_id"], request_id=state["request_id"])
        steps.append(f"Bulk plan: {plan['to_send']} to send, {plan['blocked']} blocked")
        if plan["bulk_approval_needed"]:
            steps.append(f"⚠️ Bulk approval required (>{plan['bulk_threshold']} recipients)")
    ...
    if any(w in request for w in ["enrich", "enrichment", "credit score", "bureau"]):
        ...
        log_audit_event("level3_functional", "enrich_customer", {"customer_id": customer_id}, enriched,
                        user_id=state["user_id"], request_id=state["request_id"])
        steps.append(f"Enriched {customer_id} from {len(enriched.get('enrichment', {}).get('data_sources', []))} sources")
        return _done(state, steps, enriched)

    # ── Identity gate ──
    identity = get_kyc_status(customer_id)
    if identity.get("kyc_status") == "expired" or identity.get("identity_status") == "unverified":
        ...
        log_audit_event("level3_functional", "create_case", {"customer_id": customer_id}, case,
                        user_id=state["user_id"], request_id=state["request_id"])
        steps.append(f"Identity unverified → re-verification case opened: {case['case_id']}")
        result = {"kyc_blocked": True, "case": case}
        return _done(state, steps, result)

    # ── Upsell / promotion / cross-sell / next-best-action ──
    if any(w in request for w in ["offer", "upsell", "recommend", "product", "cross-sell", "next best", "promo", "promotion"]):
        offer = recommend_offer(customer_id)
        log_audit_event("level3_functional", "recommend_offer", {"customer_id": customer_id}, offer,
                        user_id=state["user_id"], request_id=state["request_id"])
        steps.append(f"NBA promotion: {offer.get('offer_code')} (confidence {offer.get('confidence', 0):.0%})")

        if offer.get("offer_code"):
            draft = draft_email(customer_id, "T-PROMO-01",
                                variables={"offer_name": offer["offer_name"]})
            log_audit_event("level3_functional", "draft_email", {"customer_id": customer_id}, {},
                            user_id=state["user_id"], request_id=state["request_id"])
            steps.append("Email drafted")

            send = send_notification(customer_id, "email",
                                     {"subject": draft.get("subject"), "body": draft.get("body")})
            log_audit_event("level3_functional", "send_notification",
                            {"customer_id": customer_id, "channel": "email"}, send,
                            user_id=state["user_id"], request_id=state["request_id"])
            steps.append(f"Notification: {send['status']}")
            result = {"offer": offer, "send": send}

    # ── Win-back / churn prevention ──
    elif any(w in request for w in ["payment", "reminder", "collection", "delay", "winback", "win-back", "churn"]):
        ...
        if return_risk > cfg.payment_delay_threshold:
            ...
            log_audit_event("level3_functional", "request_human_approval",
                            {"customer_id": customer_id, "return_risk": return_risk}, approval,
                            user_id=state["user_id"], request_id=state["request_id"])
            if not approval["approved"]:
                return _error(state, "Human approval rejected for high-risk win-back action")
            steps.append(f"Approval granted by {approval['approver_id']}")

        send = send_notification(customer_id, "sms", {"body": "We miss you! Here's a special offer to come back."})
        log_audit_event("level3_functional", "send_notification",
                        {"customer_id": customer_id, "channel": "sms"}, send,
                        user_id=state["user_id"], request_id=state["request_id"])
...
def _done(state: AgentState, steps: list[str], result: dict) -> dict:
    ...
    return {
        ...
        "tool_calls": [{"tool": "level3", "inputs": {"request": state["original_request"]}, "outputs": result}],
        "audit_trail": [{"node": "level3_functional", "action": "completed", "steps": steps}],
    }

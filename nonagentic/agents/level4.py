...
from nonagentic.tools.monitoring import get_kpi_report, record_campaign_outcome, reflect_and_replan
...
def _decompose(goal: str) -> list[str]:
    try:
        ...
        resp = llm.invoke([
            SystemMessage(content="Break the business goal into 2-4 concrete sub-tasks. Return a numbered list only."),
...
def _run_campaign_cycle(
    ...
    steps.append(f"Segmentation: {len(segments)} segments, top '{segment_id}' ({estimated_reach} customers)")
    log_audit_event("level4_strategic", "generate_segment", {"filters": seg_filters}, seg_result,
    ...
    log_audit_event("level4_strategic", "get_kpi_report", {"segment_id": segment_id}, kpi,
                    user_id=state["user_id"], request_id=state["request_id"])
    ...
    if estimated_reach > cfg.campaign_reach_threshold:
        ...
        log_audit_event("level4_strategic", "request_human_approval",
                        {"estimated_reach": estimated_reach}, approval,
                        user_id=state["user_id"], request_id=state["request_id"])
        approved = approval["approved"]
        steps.append(f"Approval: {'granted' if approved else 'rejected'}")

    if not approved:
        return {"segment": segment_id, "campaign": None, "blocked": "approval_rejected", "kpi_baseline": kpi}
    ...
    log_audit_event("level4_strategic", "schedule_campaign", {"segment_id": segment_id}, campaign,
                    user_id=state["user_id"], request_id=state["request_id"])
...
def level4_node(state: AgentState) -> dict:
    ...
    log_audit_event("level4_strategic", "decompose_goal", {"goal": goal}, {"sub_goals": sub_goals},
                    user_id=state["user_id"], request_id=state["request_id"])
    ...
    return {
        ...
        "audit_trail": [{"node": "level4_strategic", "action": "completed", "steps": steps}],
    }

...
Usage:
    from nonagentic.config import cfg
...
class Config:
    # Approval thresholds
    bulk_notification_threshold: (
        int  # send_notification: require approval if recipients > this
    )
    campaign_reach_threshold: int  # schedule_campaign: require approval if reach > this
    risk_score_threshold: (
        float  # level3: require approval if customer fraud_score > this
    )
    ...
    target_conversion_rate: (
        float  # level4: default target conversion rate for campaigns
    )

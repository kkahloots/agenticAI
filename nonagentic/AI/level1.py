...
def _rag_generate(question: str, chunks: list[dict]) -> str:
    ...
    try:
        ...
        lines = [
            f"[{i+1}] [{c['source']}] {c['text'][:200]}" for i, c in enumerate(chunks)
        ]
        return "**Sources:**\n" + "\n".join(lines)
...
def level1_node(state: AgentState) -> dict:
    ...
    if (
    ...
    elif customer_id or any(
        w in request for w in ["profile", "account", "segment", "engagement"]
    ):
        ...
        doc_type = None
        if any(
            w in request for w in ["identity", "verification", "fraud", "compliance"]
        ):
            doc_type = "policy"

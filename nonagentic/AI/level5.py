...
def level5_node(state: AgentState) -> dict:
    ...
    if not customer_id:
        return {
            ...
            "messages": [
                AIMessage(
                    content="❌ Please provide a customer ID to generate recommendations."
                )
            ],
            "audit_trail": [
                {"node": "level5_recommendation", "action": "missing_customer_id"}
            ],
        }
    ...
    return {
        ...
        "tool_calls": [
            {
                "tool": "recommend",
                "inputs": {"customer_id": customer_id, "top_k": top_k},
                "outputs": result,
            }
        ],
        "audit_trail": [
            {
                "node": "level5_recommendation",
                "action": "completed",

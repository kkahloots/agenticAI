...
def search_policy_docs(
    query: str, doc_type: Optional[str] = None, top_k: int = 5
) -> dict:
        ...
        chunks = [
            {
                ...
                "doc_type": meta.get("doc_type", doc_type or "policy"),
                "score": dist,
            }
...
def multi_query_search(
    ...
    for col_name in collections_to_search:
        for q in queries:
            for chunk in search_policy_docs(
                q, doc_type=_COLLECTION_TO_DOC_TYPE.get(col_name), top_k=top_k
            ).get("chunks", []):
...
def ingest_documents(docs: list[dict]) -> None:
    ...
    for col_name, group in groups.items():
        ...
        collection.upsert(
            ...
            metadatas=[
                {"source": d["source"], "doc_type": d.get("doc_type", "general")}
                for d in group
            ],
        )

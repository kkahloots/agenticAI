...
_OUTCOMES_PATH = os.getenv(
    "OUTCOMES_PATH",
    str(Path(__file__).parent.parent.parent / "data" / "campaign_outcomes.jsonl"),
)
            ...
            for line in f:
                ...
                _KPI_STORE[seg] = {
                    ...
                    "avg_conversion_rate": round(
                        (prior["avg_conversion_rate"] * (n - 1) + conv) / n, 4
                    ),
                    "avg_open_rate": round(
                        (prior["avg_open_rate"] * (n - 1) + open_r) / n, 4
                    ),
...
def record_campaign_outcome(
    ...
    conversion_rate = (
        actual_conversions / estimated_reach if estimated_reach > 0 else 0.0
    )
    open_rate = actual_opens / estimated_reach if estimated_reach > 0 else 0.0

    prior = _KPI_STORE.get(
        segment_id,
        {"campaigns_run": 0, "avg_conversion_rate": 0.0, "avg_open_rate": 0.0},
    )
    n = prior["campaigns_run"] + 1
    _KPI_STORE[segment_id] = {
        "campaigns_run": n,
        "avg_conversion_rate": round(
            (prior["avg_conversion_rate"] * (n - 1) + conversion_rate) / n, 4
        ),
...
def check_kpi_deviation(
    segment_id: str, target_conversion_rate: float | None = None
) -> dict:
    from nonagentic.core.config import load_config
    ...
    target = (
        ...
        else cfg.target_conversion_rate
    )
    ...
    if should_replan:
        return {
            ...
            "reason": (
                ...
                f"threshold {cfg.kpi_deviation_threshold:.1%}"
                if deviation_report["action_required"]
...
def _has_pending_reanalysis(segment_id: str) -> bool:
    ...
    try:
        ...
        for line in reversed(lines[-10:]):
            ...
            if record.get("segment_id") == segment_id and record.get(
                "needs_reanalysis"
            ):
                return True
    except Exception:

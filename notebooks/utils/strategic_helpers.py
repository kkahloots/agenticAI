...
def run_uc2_orchestration(goal: str) -> dict:
    ...
    """
    import src.tools.monitoring as _mon
    from src.tools.analytics import generate_segment
    from src.tools.monitoring import get_kpi_report
    from src.tools.approval import request_human_approval, set_auto_approve
    from src.tools.strategic import schedule_campaign
    from .visualization import (
        display_title, display_status_banner, display_kpi_history,
        display_strategic_orchestration,
    )
    from .display_helpers import display_metrics

    # Fix outcomes path relative to project root
    _mon._OUTCOMES_PATH = str(_root / "data" / "campaign_outcomes.jsonl")
    ...
    display_metrics({"Segments Found": len(segments), "Top Segment": segment_id, "Reach": estimated_reach})

    # Step 2 — KPI baseline
    kpi = get_kpi_report(segment_id)
    display_title("Step 2 — KPI Baseline", emoji="📊")
    display_metrics({"Campaigns Run": kpi["campaigns_run"],
                     "Avg Conversion": f"{kpi['avg_conversion_rate']:.1%}",
                     "Summary": kpi["summary"]})
    display_kpi_history(str(_root / "data" / "campaign_outcomes.jsonl"), segment_id)
    ...
    display_status_banner(
        f"✅ Approved by: {approval['approver_id']}" if approval["approved"] else "🚫 Approval rejected",
    ...
    if approval["approved"]:
        ...
        display_strategic_orchestration({
            "segment": segment_id, "campaign": campaign,
            "kpi_baseline": kpi, "reflection": {"should_replan": False},
        })

    return {"segments": segments, "segment_id": segment_id,
            "estimated_reach": estimated_reach, "kpi": kpi,
            "approval": approval, "campaign": campaign}


def run_uc5_pilot(segments: list) -> dict:
    """Run UC5 pilot test + compliance failure example."""
    from src.tools.strategic import run_pilot_test
    from .visualization import display_pilot_results, display_title

    vip_segs = [s for s in segments if s["label"] == "vip"]
    vip_ids = vip_segs[0]["customer_ids"] if vip_segs else [f"CUST-{i:03d}" for i in range(1, 46)]
...
def run_uc7_reflection(segment_id: str, goal: str, kpi: dict, campaign: dict) -> dict:
    ...
    from src.tools.monitoring import check_kpi_deviation, reflect_and_replan
    from src.agents.level4 import _decompose
    from .visualization import (
        display_title, display_kpi_deviation,
        display_strategic_orchestration, display_goal_decomposition,
    )
    ...
    display_strategic_orchestration({
        "segment": segment_id, "campaign": campaign,
        "kpi_baseline": kpi, "reflection": reflection,
    })
...
def run_uc8_governance(goal: str, sub_goals: list, segments: list,
                       segment_id: str, campaign: dict, reflection: dict) -> None:
    ...
    log_audit_event("level4_strategic", "decompose_goal",
                    {"goal": goal}, {"sub_goals": sub_goals},
                    user_id="demo-user", request_id=request_id)
    log_audit_event("level4_strategic", "generate_segment",
                    {"filters": None}, {"segments": len(segments)},
                    user_id="demo-user", request_id=request_id)
    log_audit_event("level4_strategic", "schedule_campaign",
                    {"segment_id": segment_id}, {"status": "scheduled"},
                    user_id="demo-user", request_id=request_id)
    log_audit_event("level4_strategic", "reflect_and_replan",
                    {"segment_id": segment_id}, reflection,
                    user_id="demo-user", request_id=request_id)

    display_governance_dashboard(
        audit_path=str(_root / "data" / "audit.jsonl"),

...
# Get project root - notebooks/nonagent/utils/notebook_helpers.py -> agenticAI (4 parents)
...
def show_result(result, show_chunks=False, title="Answer"):
    ...
    if show_chunks and "result" in result:
        ...
        if chunks:
            display(
                HTML(
                    '<div style="margin-top:16px;font-weight:600;color:#475569">📚 Retrieved Sources</div>'
                )
            )
...
def display_customer_profile(customer_data, title="Customer Profile"):
    ...
    if optional:
        ...
        display_card(
            title,
            f'<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;font-size:14px">{rows}</div>',
            emoji="👤",
            color="#3b82f6",
        )
    ...
    if customer_data.get("purchase_categories"):
        ...
        display(
            HTML(
                '<div style="margin-top:16px"><strong>Purchase Categories</strong></div>'
            )
        )
        display(
            categories_df.style.hide(axis="index").set_table_styles(
                [
                    {
                        "selector": "th",
                        "props": [("background", "#3b82f6"), ("color", "white")],
                    },
    ...
    if customer_data.get("consent_flags"):
        ...
        display(
            HTML('<div style="margin-top:16px"><strong>Consent Flags</strong></div>')
        )
        display(
            consent_df.style.hide(axis="index").set_table_styles(
                [
                    {
                        "selector": "th",
                        "props": [("background", "#10b981"), ("color", "white")],
                    },
                    {"selector": "td", "props": [("padding", "6px")]},
                ]
            )
...
def display_identity_status_analysis(
    customers_to_check, title="Identity Verification Status Analysis"
):
    ...
    display(
        HTML(
            f'<div style="font-size:18px;font-weight:600;margin:16px 0">🔐 {title}</div>'
        )
    )
    ...
    db_path = _PROJECT_ROOT / "data" / "customers.db"
    ...
    for cid in customers_to_check:
        ...
        identity_results.append(
            {"Customer": cid, "Status": row[1], "Fraud Score": fraud_score}
        )
...
def display_search_results(
    result, result_type="Search Results", emoji="📧", color="#8b5cf6"
):
...
def display_policy_qa(questions, title="Policy Q&A"):
    ...
    for i, q in enumerate(questions, 1):
        ...
        display(
            HTML(
                f'<div style="font-size:16px;font-weight:600;margin:20px 0 8px 0">📋 Policy Question {i}</div>'
            )
        )
        display(
            HTML(
                f'<div style="color:#64748b;font-style:italic;margin-bottom:12px">{q}</div>'
            )
        )
        display_card("Policy Answer", answer, emoji="📖", color="#3b82f6")

        # Show source documents (chunks can be in result["result"] or result directly)
        chunks = result.get("result", {}).get("chunks", []) or result.get("chunks", [])
        if chunks:
            sources = list(set(c["source"] for c in chunks))
            sources_html = ", ".join(
                [
                    f'<code style="background:#f1f5f9;padding:2px 6px;border-radius:4px">{s}</code>'
                    for s in sources
                ]
            )
            display(
                HTML(
                    f'<div style="margin-top:8px;font-size:13px;color:#64748b">📚 Sources: {sources_html}</div>'
                )
            )
...
def run_guardrail_demo(test_output=None):
    ...
    display(
        HTML(
            '<div style="margin-top:16px;padding:12px;background:#f0fdf4;border-left:4px solid #10b981;border-radius:4px;color:#065f46">✅ Guardrails disabled for remaining demos</div>'
        )
    )

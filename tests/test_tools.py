...
from src.tools.functional import send_notification, create_case, recommend_offer
...
def test_send_notification_blocked_when_no_marketing_consent(monkeypatch):
    ...
    fake = {
        ...
        "consent_flags": {"marketing": False, "email": True, "sms": True, "data_sharing": True},
    }
...
def test_send_notification_blocked_no_sms_consent(monkeypatch):
    ...
    fake = {
        "customer_id": "CUST-TEST",
        "consent_flags": {"marketing": True, "email": True, "sms": False, "data_sharing": True},
    }
    monkeypatch.setattr(cmod, "_find", lambda *a, **kw: fake)
...
def test_generate_segment_insufficient_data():
    result = generate_segment(filters={"kyc_status": "nonexistent_status"}, algorithm="rules")
...
def test_schedule_campaign_succeeds_when_approved():
    result = schedule_campaign(
        ...
        segment_id="new",
        steps=[{"channel": "sms", "template_id": "T-PAYMENT-REMINDER", "delay_days": 0}],
        start_date="2025-08-01",
        estimated_reach=50,
        approved=True,
    )
...
def test_log_audit_event_returns_logged(tmp_path, monkeypatch):
    ...
    import importlib, src.tools.audit as amod
    importlib.reload(amod)
    result = amod.log_audit_event("test_agent", "test_action", {"key": "val"}, {"out": 1})
...
def test_audit_masks_pii(tmp_path, monkeypatch):
    ...
    import importlib, src.tools.audit as amod
    importlib.reload(amod)
    amod.log_audit_event("agent", "action", {"email": "real@email.com", "safe": "ok"}, {})
